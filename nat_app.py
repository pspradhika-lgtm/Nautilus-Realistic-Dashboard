...
f = df.query(
    ...
    "(@i==[] or Incident_Type in @i)"
) if (
    (y:=st.sidebar.multiselect("Year", sorted(df.Year.dropna().unique()))) or
    (m:=st.sidebar.multiselect("Month", df.Month_Name.unique())) or
    (c:=st.sidebar.multiselect("Country", sorted(df.Country.dropna().unique()))) or
    (v:=st.sidebar.multiselect("Vessel", sorted(df.Vessel_Type.dropna().unique()))) or
...
f = f.query("@r[0] <= Casualties <= @r[1]")
st.sidebar.success(f"📊 {len(f)} records")
...
with t1:
    if f.empty: st.warning("No data.")
    else:
        m=folium.Map([20,0],2); cl=MarkerCluster().add_to(m)
...
with t4:
    if not f.empty:
        ...
        fig=go.Figure(); 
        [fig.add_trace(go.Scatterpolar(r=v,theta=g.columns,fill='toself',name=k)) for k,v in g.iterrows()]
        st.plotly_chart(fig, use_container_width=True)

with t5:
    if not f.empty:
        st.plotly_chart(px.sunburst(f,path=["Incident_Type","Vessel_Type","Country"],title="Sunburst"),use_container_width=True)
        st.plotly_chart(px.funnel(f.Severity.value_counts().reset_index(),x="count",y="index",title="Severity Funnel"),use_container_width=True)
